...
import { IDoctorSchedule } from "@/types/doctor";
...
const DoctorScheduleSlots = ({ id }: { id: string }) => {
  ...
  query.startDate = dayjs(nextDate).utc().hour(0).minute(0).second(0).millisecond(0).toISOString();
  ...
  const { data: nextDoctorSchedules, isLoading: loading } = useGetAllDoctorSchedulesQuery({
    ...query,
  });
  const schedulesOfTomorrow = nextDoctorSchedules?.doctorSchedules;

  const availableSlots = doctorSchedules?.filter((doctor: IDoctorSchedule) => !doctor.isBooked);
  ...
  return (
    <Box mb={5}>
      <Box sx={{ bgcolor: "white", p: 3, mt: 1 }}>
        ...
        <Stack direction="row" alignItems="center" flexWrap="wrap" gap={2}>
          {availableSlots?.length ? (
              ...
              availableSlots?.map((doctorSchedule: IDoctorSchedule) => {
                ...
                return (
                  <Button
                    ...
                    variant={`${
                      doctorSchedule?.scheduleId === scheduleId ? "contained" : "outlined"
                    }`}>
                    {formattedTimeSlot}
                  </Button>
                );
            ...
            <span style={{ color: "red" }}>No Schedule is Available Today!</span>
          )}
        </Stack>
        <Typography variant="h6" fontSize={16} mt={5}>
          <b>Tomorrow: {dateFormatter(nextDate.toISOString()) + " " + tomorrow}</b>
        </Typography>
        <Box sx={{ borderBottom: "2px dashed #d0d0d0", mt: 2, mb: 3 }} />
        <Stack direction="row" alignItems="center" flexWrap="wrap" gap={2}>
      ...
      <Button onClick={handleBookAppointment} sx={{ display: "block", mx: "auto" }}>
