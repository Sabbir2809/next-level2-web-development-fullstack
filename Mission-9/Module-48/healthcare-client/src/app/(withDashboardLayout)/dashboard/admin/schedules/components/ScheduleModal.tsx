...
const ScheduleModal = ({ open, setOpen }: TProps) => {
  ...
  const handleSubmit = async (values: FieldValues) => {
    values.startDateTime = dateFormatter(values.startDateTime);
    values.endDateTime = dateFormatter(values.endDateTime);
    ...
    <HModal open={open} setOpen={setOpen} title="Create Schedule">
      <HForm onSubmit={handleSubmit}>
        <Grid container spacing={2} sx={{ width: "400px" }}>
          <Grid item md={6}>
            <HDatePicker name="startDateTime" label="Start Date" />
          </Grid>
          <Grid item md={6}>
            <HDatePicker name="endDateTime" label="End Date" />
          </Grid>
          <Grid item md={6}>
