...
const getFacultiesWithCourseFormDB = async (courseId: string) => {
  const result = await CourseFaculty.findOne({ courseId }).populate("faculties");
  return result;
};
