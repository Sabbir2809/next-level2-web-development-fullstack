...
const CreateFaculty = () => {
  ...
  const { data: fData, isLoading: fIsLoading } = useGetAcademicDepartmentsQuery(undefined);

  const departmentOptions = fData?.data?.map((item) => ({
    value: item._id,
    label: item.name,
  }));
  ...
  return (
    <Row>
      <Col span={24}>
        <UMForm onSubmit={onSubmit} defaultValues={facultyData}>
